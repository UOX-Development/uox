...
#define JSSTRING_CHARS(str)                                                    \
    (JSSTRING_IS_DEPENDENT(str) ? JSSTRDEP_CHARS(str) : (str)->chars)
#define JSSTRING_LENGTH(str)                                                   \
    (JSSTRING_IS_DEPENDENT(str) ? JSSTRDEP_LENGTH(str) : (str)->length)
#define JSSTRING_LENGTH_BITS                                                   \
    (sizeof(size_t) * JS_BITS_PER_BYTE - JSSTRFLAG_BITS)
...
#define JSSTRDEP_START(str)                                                    \
    (JSSTRING_IS_PREFIX(str)                                                   \
         ? 0                                                                   \
         : ((JSSTRDEP(str)->length >> JSSTRDEP_START_SHIFT) &                  \
            JSSTRDEP_START_MASK))
#define JSSTRDEP_LENGTH(str)                                                   \
    (JSSTRDEP(str)->length &                                                   \
     (JSSTRING_IS_PREFIX(str) ? JSSTRING_LENGTH_MASK : JSSTRDEP_LENGTH_MASK))

#define JSSTRDEP_SET_START_AND_LENGTH(str, off, len)                           \
    (JSSTRDEP(str)->length =                                                   \
         JSSTRFLAG_DEPENDENT | ((off) << JSSTRDEP_START_SHIFT) | (len))
...
extern size_t js_MinimizeDependentStrings(JSString *str, int level,
                                          JSString **basep);

extern jschar *js_GetDependentStringChars(JSString *str);

extern jschar *js_GetStringChars(JSString *str);

extern JSString *js_ConcatStrings(JSContext *cx, JSString *left,
                                  JSString *right);

extern const jschar *js_UndependString(JSContext *cx, JSString *str);
...
#define JS_ISALPHA(c)                                                          \
    ((((1 << JSCT_UPPERCASE_LETTER) | (1 << JSCT_LOWERCASE_LETTER) |           \
       (1 << JSCT_TITLECASE_LETTER) | (1 << JSCT_MODIFIER_LETTER) |            \
       (1 << JSCT_OTHER_LETTER)) >>                                            \
      JS_CTYPE(c)) &                                                           \
     1)
...
#define JS_ISALNUM(c)                                                          \
    ((((1 << JSCT_UPPERCASE_LETTER) | (1 << JSCT_LOWERCASE_LETTER) |           \
       (1 << JSCT_TITLECASE_LETTER) | (1 << JSCT_MODIFIER_LETTER) |            \
       (1 << JSCT_OTHER_LETTER) | (1 << JSCT_DECIMAL_DIGIT_NUMBER)) >>         \
      JS_CTYPE(c)) &                                                           \
     1)
...
#define JS_ISLETTER(c)                                                         \
    ((((1 << JSCT_UPPERCASE_LETTER) | (1 << JSCT_LOWERCASE_LETTER) |           \
       (1 << JSCT_TITLECASE_LETTER) | (1 << JSCT_MODIFIER_LETTER) |            \
       (1 << JSCT_OTHER_LETTER) | (1 << JSCT_LETTER_NUMBER)) >>                \
      JS_CTYPE(c)) &                                                           \
     1)
...
#define JS_TOUPPER(c)                                                          \
    ((jschar)((JS_CCODE(c) & 0x00100000) ? (c) - ((int32)JS_CCODE(c) >> 22)    \
                                         : (c)))
#define JS_TOLOWER(c)                                                          \
    ((jschar)((JS_CCODE(c) & 0x00200000) ? (c) + ((int32)JS_CCODE(c) >> 22)    \
                                         : (c)))
...
extern JSBool js_InitRuntimeStringState(JSContext *cx);

extern void js_FinishRuntimeStringState(JSContext *cx);

extern void js_FinishDeflatedStringCache(JSRuntime *rt);
...
extern JSString *js_NewString(JSContext *cx, jschar *chars, size_t length,
                              uintN gcflag);

extern JSString *js_NewDependentString(JSContext *cx, JSString *base,
                                       size_t start, size_t length,
                                       uintN gcflag);

/* Copy a counted string and GC-allocate a descriptor for it. */
extern JSString *js_NewStringCopyN(JSContext *cx, const jschar *s, size_t n,
                                   uintN gcflag);

/* Copy a C string and GC-allocate a descriptor for it. */
extern JSString *js_NewStringCopyZ(JSContext *cx, const jschar *s,
                                   uintN gcflag);

/* Free the chars held by str when it is finalized by the GC. */
extern void js_FinalizeString(JSContext *cx, JSString *str);

extern void js_FinalizeStringRT(JSRuntime *rt, JSString *str);

/* Wrap a string value in a String object. */
extern JSObject *js_StringToObject(JSContext *cx, JSString *str);
 ...
 */
extern JS_FRIEND_API(JSString *) js_ValueToString(JSContext *cx, jsval v);

/*
 * Convert a value to its source expression, returning null after reporting
 * an error, otherwise returning a new string reference.
 */
extern JS_FRIEND_API(JSString *) js_ValueToSource(JSContext *cx, jsval v);

#ifdef HT_ENUMERATE_NEXT /* XXX don't require jshash.h */
/*
 * Compute a hash function from str.
 */
extern JSHashNumber js_HashString(JSString *str);
#endif
 ...
 */
extern intN js_CompareStrings(JSString *str1, JSString *str2);

/*
 * Test if strings are equal.
 */
extern JSBool js_EqualStrings(JSString *str1, JSString *str2);
...
extern jsint js_BoyerMooreHorspool(const jschar *text, jsint textlen,
                                   const jschar *pat, jsint patlen,
                                   jsint start);

extern size_t js_strlen(const jschar *s);

extern jschar *js_strchr(const jschar *s, jschar c);

extern jschar *js_strchr_limit(const jschar *s, jschar c, const jschar *limit);
 ...
 */
extern const jschar *js_SkipWhiteSpace(const jschar *s);
 ...
 * jschars.
 */
extern jschar *js_InflateString(JSContext *cx, const char *bytes,
                                size_t *length);

extern char *js_DeflateString(JSContext *cx, const jschar *chars,
                              size_t length);
 ...
 */
extern JSBool js_InflateStringToBuffer(JSContext *cx, const char *bytes,
 ...
 */
extern JSBool js_DeflateStringToBuffer(JSContext *cx, const jschar *chars,
 ...
 */
extern JSBool js_SetStringBytes(JSRuntime *rt, JSString *str, char *bytes,
                                size_t length);
 ...
 */
extern char *js_GetStringBytes(JSRuntime *rt, JSString *str);

/* Remove a deflated string cache entry associated with str if any. */
extern void js_PurgeDeflatedStringCache(JSRuntime *rt, JSString *str);
 ...
 */
extern int js_OneUcs4ToUtf8Char(uint8 *utf8Buffer, uint32 ucs4Char);

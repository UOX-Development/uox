...
#define SET_JUMP_OFFSET(pc, off)                                               \
    ((pc)[1] = JUMP_OFFSET_HI(off), (pc)[2] = JUMP_OFFSET_LO(off))
...
#define SET_SPANDEP_INDEX(pc, i)                                               \
    ((pc)[1] = JUMP_OFFSET_HI(i), (pc)[2] = JUMP_OFFSET_LO(i))
...
#define GET_JUMPX_OFFSET(pc)                                                   \
    ((int32)(((pc)[1] << 24) | ((pc)[2] << 16) | ((pc)[3] << 8) | (pc)[4]))
#define SET_JUMPX_OFFSET(pc, off)                                              \
    ((pc)[1] = JUMPX_OFFSET_B3(off), (pc)[2] = JUMPX_OFFSET_B2(off),           \
     (pc)[3] = JUMPX_OFFSET_B1(off), (pc)[4] = JUMPX_OFFSET_B0(off))
...
#define SET_ATOM_INDEX(pc, i)                                                  \
    ((pc)[1] = ATOM_INDEX_HI(i), (pc)[2] = ATOM_INDEX_LO(i))
#define GET_ATOM(cx, script, pc)                                               \
    js_GetAtom((cx), &(script)->atomMap, GET_ATOM_INDEX(pc))
...
#define SET_UINT24(pc, i)                                                      \
    ((pc)[1] = UINT24_HI(i), (pc)[2] = UINT24_MID(i), (pc)[3] = UINT24_LO(i))
 ...
 */
extern JSString *js_QuoteString(JSContext *cx, JSString *str, jschar quote);
 ...
 * a GC'ed string.
 */
extern JSPrinter *js_NewPrinter(JSContext *cx, const char *name, uintN indent,
                                JSBool pretty);

extern void js_DestroyPrinter(JSPrinter *jp);

extern JSString *js_GetPrinterOutput(JSPrinter *jp);

extern int js_printf(JSPrinter *jp, const char *format, ...);

extern JSBool js_puts(JSPrinter *jp, const char *s);
 ...
 */
extern JSBool js_DecompileCode(JSPrinter *jp, JSScript *script, jsbytecode *pc,
                               uintN len, uintN pcdepth);

extern JSBool js_DecompileScript(JSPrinter *jp, JSScript *script);

extern JSBool js_DecompileFunctionBody(JSPrinter *jp, JSFunction *fun);

extern JSBool js_DecompileFunction(JSPrinter *jp, JSFunction *fun);
 ...
 */
extern JSString *js_DecompileValueGenerator(JSContext *cx, intN spindex,
                                            jsval v, JSString *fallback);

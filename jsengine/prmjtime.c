...
#define IS_LEAP(year)                                                          \
    (year != 0 &&                                                              \
     ((((year & 0x3) == 0) && ((year - ((year / 100) * 100)) != 0)) ||         \
      (year - ((year / 400) * 400)) == 0))
...
#endif
#ifdef XP_WIN
    JSInt64 s, us, win2un = JSLL_INIT(0x19DB1DE, 0xD53E8000),
                   ten = JSLL_INIT(0, 10);
...
static void PRMJ_basetime(JSInt64 tsecs, PRMJTime *prtm) {
    ...
    /* get the year */
    while ((isleap == 0) ? !JSLL_CMP(tsecs, <, result)
                         : !JSLL_CMP(tsecs, <, result2)) {
    ...
    while (((month == 1 && isleap) ? (mday >= mtab[month] + 1)
                                   : (mday >= mtab[month]))) {
        yday += mtab[month];
        days += mtab[month];

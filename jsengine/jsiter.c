...
static JSBool InitNativeIterator(JSContext *cx, JSObject *iterobj,
                                 JSObject *obj, uintN flags) {
    jsval state;
    JSBool ok;
    ...
    ok =
        ...
        ((flags & JSITER_FOREACH) && OBJECT_IS_XML(cx, obj))
            ? ((JSXMLObjectOps *)obj->map->ops)
                  ->enumerateValues(cx, obj, JSENUMERATE_INIT, &state, NULL,
                                    NULL)
            :
#endif
...
static JSBool Iterator(JSContext *cx, JSObject *iterobj, uintN argc,
...
static JSBool IteratorNextImpl(JSContext *cx, JSObject *obj, jsval *rval) {
    ...
    foreach
        = (flags & JSITER_FOREACH) != 0;
    ok =
#if JS_HAS_XML_SUPPORT
        (foreach && OBJECT_IS_XML(cx, iterable))
            ? ((JSXMLObjectOps *)iterable->map->ops)
                  ->enumerateValues(cx, iterable, JSENUMERATE_NEXT, &state, &id,
                                    rval)
            :
#endif
...
static JSBool iterator_next(JSContext *cx, JSObject *obj, uintN argc,
                            jsval *argv, jsval *rval) {
...
static JSBool iterator_self(JSContext *cx, JSObject *obj, uintN argc,
                            jsval *argv, jsval *rval) {
        ...
        if (JSVAL_IS_PRIMITIVE(*vp)) {
            ...
            if (str) {
                JS_ReportErrorNumberUC(
                    cx, js_GetErrorMessage, NULL, JSMSG_BAD_ITERATOR_RETURN,
                    JSSTRING_CHARS(str), JSSTRING_CHARS(ATOM_TO_STRING(atom)));
            }
            goto bad;
        }
    }
...
static JSBool CallEnumeratorNext(JSContext *cx, JSObject *iterobj, uintN flags,
                                 jsval *rval) {
    ...
    if (obj == origobj && OBJECT_IS_XML(cx, obj)) {
        if (foreach) {
            ...
            if (!xmlops->enumerateValues(cx, obj, JSENUMERATE_NEXT, &state, &id,
                                         rval)) {
                return JS_FALSE;
            }
        } else {
    ...
    restart:
        ...
        if (obj != obj2) {
            ...
            if (clasp->flags & JSCLASS_IS_EXTENDED) {
                ...
                cond =
                    xclasp->outerObject && xclasp->outerObject(cx, obj2) == obj;
            }
            if (!cond)
                goto restart;
        }
...
JSBool js_CallIteratorNext(JSContext *cx, JSObject *iterobj, jsval *rval) {
    ...
    if (OBJ_GET_CLASS(cx, iterobj) == &js_IteratorClass) {
        ...
        if (!js_InternalCall(cx, iterobj, *rval, 0, NULL, rval)) {
            ...
            if (!cx->throwing || JSVAL_IS_PRIMITIVE(cx->exception) ||
...
static JSBool stopiter_hasInstance(JSContext *cx, JSObject *obj, jsval v,
                                   JSBool *bp) {
...
JSClass js_GeneratorClass = {js_Generator_str,
 ...
 */
JSObject *js_NewGenerator(JSContext *cx, JSStackFrame *fp) {
    JSObject *obj;
    ...
    gen = (JSGenerator *)JS_malloc(cx, sizeof(JSGenerator) +
 ...
 */
static JSBool SendToGenerator(JSContext *cx, JSGeneratorOp op, JSObject *obj,
 ...
 */
static JSBool generator_op(JSContext *cx, JSGeneratorOp op, JSObject *obj,
    ...
    arg = (op == JSGENOP_SEND || op == JSGENOP_THROW) ? argv[0] : JSVAL_VOID;
    if (!SendToGenerator(cx, op, obj, gen, arg, rval))
        return JS_FALSE;
    return JS_TRUE;
}

static JSBool generator_send(JSContext *cx, JSObject *obj, uintN argc,
                             jsval *argv, jsval *rval) {
    return generator_op(cx, JSGENOP_SEND, obj, argc, argv, rval);
}

static JSBool generator_next(JSContext *cx, JSObject *obj, uintN argc,
                             jsval *argv, jsval *rval) {
    return generator_op(cx, JSGENOP_NEXT, obj, argc, argv, rval);
}

static JSBool generator_throw(JSContext *cx, JSObject *obj, uintN argc,
                              jsval *argv, jsval *rval) {
    return generator_op(cx, JSGENOP_THROW, obj, argc, argv, rval);
}

static JSBool generator_close(JSContext *cx, JSObject *obj, uintN argc,
                              jsval *argv, jsval *rval) {
...
JSObject *js_InitIteratorClasses(JSContext *cx, JSObject *obj) {
    JSObject *proto, *stop;
    ...
    if (!JS_InitClass(cx, obj, NULL, &js_GeneratorClass, NULL, 0, NULL,
                      generator_methods, NULL, NULL)) {
        return NULL;
    }
#endif

    return JS_InitClass(cx, obj, NULL, &js_StopIterationClass, NULL, 0, NULL,
                        NULL, NULL, NULL);
}

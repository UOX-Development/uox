...
extern void js_DropProperty(JSContext *cx, JSObject *obj, JSProperty *prop);
#else
#define NATIVE_DROP_PROPERTY NULL
#endif

JS_FRIEND_DATA(JSObjectOps)
js_ObjectOps = {js_NewObjectMap,
                js_DestroyObjectMap,
                js_LookupProperty,
                js_DefineProperty,
                js_GetProperty,
...
JSClass js_ObjectClass = {js_Object_str,
...
static JSBool obj_getSlot(JSContext *cx, JSObject *obj, jsval id, jsval *vp);

static JSBool obj_setSlot(JSContext *cx, JSObject *obj, jsval id, jsval *vp);

static JSBool obj_getCount(JSContext *cx, JSObject *obj, jsval id, jsval *vp);

static JSPropertySpec object_props[] = {
    /* These two must come first; see object_props[slot].name usage below. */
    {js_proto_str, JSSLOT_PROTO, JSPROP_PERMANENT | JSPROP_SHARED, obj_getSlot,
     obj_setSlot},
    {js_parent_str, JSSLOT_PARENT,
     JSPROP_READONLY | JSPROP_PERMANENT | JSPROP_SHARED, obj_getSlot,
     obj_setSlot},
...
static JSBool ReportStrictSlot(JSContext *cx, uint32 slot) {
    if (slot == JSSLOT_PROTO)
        return JS_TRUE;
    return JS_ReportErrorFlagsAndNumber(
        cx, JSREPORT_WARNING | JSREPORT_STRICT, js_GetErrorMessage, NULL,
        JSMSG_DEPRECATED_USAGE, object_props[slot].name);
}
...
static JSBool obj_setSlot(JSContext *cx, JSObject *obj, jsval id, jsval *vp) {
    ...
    propid = ATOM_TO_JSID(cx->runtime->atomState.protoAtom);
    if (!OBJ_CHECK_ACCESS(cx, obj, propid, JSACC_PROTO | JSACC_WRITE, vp,
                          &attrs))
        return JS_FALSE;
...
JSBool js_SetProtoOrParent(JSContext *cx, JSObject *obj, uint32 slot,
                           JSObject *pobj) {
    JSRuntime *rt;
...
static JSHashEntry *MarkSharpObjects(JSContext *cx, JSObject *obj,
                                     JSIdArray **idap) {
...
JSHashEntry *js_EnterSharpObject(JSContext *cx, JSObject *obj, JSIdArray **idap,
    ...
    *sp = NULL;
    ...
    sharpid = JS_PTR_TO_UINT32(he->value);
    if (sharpid != 0) {
        len = JS_snprintf(buf, sizeof buf, "#%u%c", sharpid >> SHARP_ID_SHIFT,
                          (sharpid & SHARP_BIT) ? '#' : '=');
...
#if JS_HAS_TOSOURCE
JSBool js_obj_toSource(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       jsval *rval) {
    JSBool ok, outermost;
    JSHashEntry *he;
    JSIdArray *ida;
    ...
    if (cx->options & JSOPTION_LOGCALL_TOSOURCE) {
        ...
        chars = (jschar *)realloc(
            (ochars = chars), (nchars + classnchars + 2 + 1) * sizeof(jschar));
        if (!chars) {
            free(ochars);
            goto error;
        }
    ...
    for (i = 0, length = ida->length; i < length; i++) {
        ...
        *rval = STRING_TO_JSVAL(idstr); /* local root */
        ...
        needOldStyleGetterSetter =
            ...
            js_CheckKeyword(JSSTRING_CHARS(idstr), JSSTRING_LENGTH(idstr)) !=
                TOK_EOF;
...
#undef SAFE_ADD
            ...
            chars =
                (jschar *)realloc((ochars = chars), curlen * sizeof(jschar));
            if (!chars) {
...
JSBool js_obj_toString(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       jsval *rval) {
    jschar *chars;
    size_t nchars;
...
static JSBool js_obj_toLocaleString(JSContext *cx, JSObject *obj, uintN argc,
...
static JSBool obj_valueOf(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          jsval *rval) {
 ...
 */
JSBool js_CheckPrincipalsAccess(JSContext *cx, JSObject *scopeobj,
...
JSObject *js_CheckScopeChainValidity(JSContext *cx, JSObject *scopeobj,
                                     const char *caller) {
    JSClass *clasp;
    JSExtendedClass *xclasp;
    JSObject *inner;
...
static JSBool obj_eval(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       jsval *rval) {
...
#endif
    {
        ...
        if (indirectCall) {
            ...
            if (obj != callerScopeChain) {
                if (!js_CheckPrincipalsAccess(
                        cx, obj, caller->script->principals,
...
static JSBool obj_watch_handler(JSContext *cx, JSObject *obj, jsval id,
                                jsval old, jsval *nvp, void *closure) {
    JSObject *callable;
    JSRuntime *rt;
    JSStackFrame *caller;
...
static JSBool obj_watch(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        jsval *rval) {
    JSObject *callable;
    jsval userid, value;
    jsid propid;
...
static JSBool obj_unwatch(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          jsval *rval) {
...
static JSBool obj_hasOwnProperty(JSContext *cx, JSObject *obj, uintN argc,
                                 jsval *argv, jsval *rval) {
    return js_HasOwnPropertyHelper(cx, obj, obj->map->ops->lookupProperty, argc,
                                   argv, rval);
}

JSBool js_HasOwnPropertyHelper(JSContext *cx, JSObject *obj,
                               JSLookupPropOp lookup, uintN argc, jsval *argv,
                               jsval *rval) {
    jsid id;
    JSObject *obj2;
    JSProperty *prop;
    ...
    if (!prop) {
        ...
        *rval = JSVAL_TRUE;
        ...
        xclasp = (clasp->flags & JSCLASS_IS_EXTENDED) ? (JSExtendedClass *)clasp
                                                      : NULL;
...
static JSBool obj_isPrototypeOf(JSContext *cx, JSObject *obj, uintN argc,
                                jsval *argv, jsval *rval) {
    JSBool b;
...
static JSBool obj_propertyIsEnumerable(JSContext *cx, JSObject *obj, uintN argc,
                                       jsval *argv, jsval *rval) {
    jsid id;
    uintN attrs;
    JSObject *obj2;
...
#if JS_HAS_GETTER_SETTER
static JSBool obj_defineGetter(JSContext *cx, JSObject *obj, uintN argc,
                               jsval *argv, jsval *rval) {
    jsval fval, junk;
    jsid id;
    uintN attrs;
    ...
    if (JS_TypeOfValue(cx, fval) != JSTYPE_FUNCTION) {
        JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
                             JSMSG_BAD_GETTER_OR_SETTER, js_getter_str);
        return JS_FALSE;
    }
    ...
    if (!OBJ_CHECK_ACCESS(cx, obj, id, JSACC_WATCH, &junk, &attrs))
        return JS_FALSE;
    return OBJ_DEFINE_PROPERTY(
        cx, obj, id, JSVAL_VOID, (JSPropertyOp)JSVAL_TO_OBJECT(fval), NULL,
        JSPROP_ENUMERATE | JSPROP_GETTER | JSPROP_SHARED, NULL);
}

static JSBool obj_defineSetter(JSContext *cx, JSObject *obj, uintN argc,
                               jsval *argv, jsval *rval) {
    jsval fval, junk;
    jsid id;
    uintN attrs;
    ...
    if (JS_TypeOfValue(cx, fval) != JSTYPE_FUNCTION) {
        JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
                             JSMSG_BAD_GETTER_OR_SETTER, js_setter_str);
        return JS_FALSE;
    }
    ...
    if (!OBJ_CHECK_ACCESS(cx, obj, id, JSACC_WATCH, &junk, &attrs))
        return JS_FALSE;
    return OBJ_DEFINE_PROPERTY(
        cx, obj, id, JSVAL_VOID, NULL, (JSPropertyOp)JSVAL_TO_OBJECT(fval),
        JSPROP_ENUMERATE | JSPROP_SETTER | JSPROP_SHARED, NULL);
}

static JSBool obj_lookupGetter(JSContext *cx, JSObject *obj, uintN argc,
                               jsval *argv, jsval *rval) {
    jsid id;
    JSObject *pobj;
    JSProperty *prop;
...
static JSBool obj_lookupSetter(JSContext *cx, JSObject *obj, uintN argc,
                               jsval *argv, jsval *rval) {
    jsid id;
    JSObject *pobj;
    JSProperty *prop;
...
static JSBool Object(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                     jsval *rval) {
    if (argc == 0) {
 ...
 */
static JSBool with_LookupProperty(JSContext *cx, JSObject *obj, jsid id,
...
static JSBool with_GetProperty(JSContext *cx, JSObject *obj, jsid id,
                               jsval *vp) {
    JSObject *proto = OBJ_GET_PROTO(cx, obj);
    if (!proto)
...
static JSBool with_SetProperty(JSContext *cx, JSObject *obj, jsid id,
                               jsval *vp) {
    JSObject *proto = OBJ_GET_PROTO(cx, obj);
    if (!proto)
...
static JSBool with_GetAttributes(JSContext *cx, JSObject *obj, jsid id,
...
static JSBool with_SetAttributes(JSContext *cx, JSObject *obj, jsid id,
                                 JSProperty *prop, uintN *attrsp) {
    JSObject *proto = OBJ_GET_PROTO(cx, obj);
    if (!proto)
...
static JSBool with_DeleteProperty(JSContext *cx, JSObject *obj, jsid id,
                                  jsval *rval) {
    JSObject *proto = OBJ_GET_PROTO(cx, obj);
    if (!proto)
...
static JSBool with_DefaultValue(JSContext *cx, JSObject *obj, JSType hint,
                                jsval *vp) {
    JSObject *proto = OBJ_GET_PROTO(cx, obj);
    if (!proto)
...
static JSBool with_Enumerate(JSContext *cx, JSObject *obj, JSIterateOp enum_op,
...
static JSBool with_CheckAccess(JSContext *cx, JSObject *obj, jsid id,
...
static JSObject *with_ThisObject(JSContext *cx, JSObject *obj) {
    JSObject *proto = OBJ_GET_PROTO(cx, obj);
    if (!proto)
        return obj;
...
JS_FRIEND_DATA(JSObjectOps)
js_WithObjectOps = {js_NewObjectMap,
                    js_DestroyObjectMap,
                    with_LookupProperty,
                    js_DefineProperty,
                    with_GetProperty,
...
static JSObjectOps *with_getObjectOps(JSContext *cx, JSClass *clasp) {
    return &js_WithObjectOps;
}

JSClass js_WithClass = {"With",
...
JSObject *js_NewWithObject(JSContext *cx, JSObject *proto, JSObject *parent,
                           jsint depth) {
    JSObject *obj;
...
JSObject *js_CloneBlockObject(JSContext *cx, JSObject *proto, JSObject *parent,
                              JSStackFrame *fp) {
    JSObject *clone;
...
JSBool js_PutBlockObject(JSContext *cx, JSObject *obj) {
    ...
    for (sprop = OBJ_SCOPE(obj)->lastProp; sprop; sprop = sprop->parent) {
        ...
        if (!js_DefineNativeProperty(cx, obj, sprop->id, fp->spbase[slot], NULL,
                                     NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT,
                                     SPROP_HAS_SHORTID, sprop->shortid, NULL)) {
...
static JSBool block_getProperty(JSContext *cx, JSObject *obj, jsval id,
                                jsval *vp) {
    JSStackFrame *fp;
    jsint slot;
...
static JSBool block_setProperty(JSContext *cx, JSObject *obj, jsval id,
                                jsval *vp) {
    JSStackFrame *fp;
    jsint slot;
...
jsatomid FindObjectAtomIndex(JSAtomMap *map, JSObject *obj) {
    size_t i;
    JSAtom *atom;
...
static JSBool block_xdrObject(JSXDRState *xdr, JSObject **objp) {
    JSContext *cx;
    jsatomid parentId;
    JSAtomMap *atomMap;
    JSObject *obj, *parent;
...
JSClass js_BlockClass = {"Block",
                         JSCLASS_HAS_PRIVATE | JSCLASS_HAS_RESERVED_SLOTS(1) |
                             JSCLASS_IS_ANONYMOUS |
                             JSCLASS_HAS_CACHED_PROTO(JSProto_Block),
                         JS_PropertyStub,
                         JS_PropertyStub,
                         block_getProperty,
                         block_setProperty,
...
JSObject *js_InitBlockClass(JSContext *cx, JSObject *obj) {
    ...
    proto = JS_InitClass(cx, obj, NULL, &js_BlockClass, NULL, 0, NULL, NULL,
                         NULL, NULL);
    if (!proto)
        return NULL;
...
JSObject *js_InitObjectClass(JSContext *cx, JSObject *obj) {
    ...
    if (!OBJ_GET_PROPERTY(
            cx, proto, ATOM_TO_JSID(cx->runtime->atomState.evalAtom), &eval)) {
        return NULL;
    }
...
void js_InitObjectMap(JSObjectMap *map, jsrefcount nrefs, JSObjectOps *ops,
                      JSClass *clasp) {
    map->nrefs = nrefs;
    map->ops = ops;
...
JSObjectMap *js_NewObjectMap(JSContext *cx, jsrefcount nrefs, JSObjectOps *ops,
...
static jsval *AllocSlots(JSContext *cx, jsval *slots, uint32 nslots) {
    ...
    if (nbytes <= GC_NBYTES_MAX) {
        ...
        newslots = (jsval *)JS_realloc(
            cx, (obytes <= GC_NBYTES_MAX) ? NULL : slots - 1, nbytes);
    }
    if (!newslots)
        return NULL;
...
static void FreeSlots(JSContext *cx, jsval *slots) {
    size_t nbytes;
...
extern JSBool js_GetClassId(JSContext *cx, JSClass *clasp, jsid *idp) {
    JSProtoKey key;
    JSAtom *atom;
...
JSObject *js_NewObject(JSContext *cx, JSClass *clasp, JSObject *proto,
    ...
    if (!proto) {
        ...
        if (!js_GetClassPrototype(cx, parent, id, &proto))
            return NULL;
        if (!proto && !js_GetClassPrototype(
                          cx, parent, INT_TO_JSID(JSProto_Object), &proto)) {
            return NULL;
        }
    }

    /* Always call the class's getObjectOps hook if it has one. */
    ops = clasp->getObjectOps ? clasp->getObjectOps(cx, clasp) : &js_ObjectOps;
...
#define JS_PROTO(name, code, init)                                             \
    extern JSObject *init(JSContext *, JSObject *);
#include "jsproto.tbl"
#undef JS_PROTO
...
JSBool js_SetClassObject(JSContext *cx, JSObject *obj, JSProtoKey key,
                         JSObject *cobj) {
...
JSBool js_FindClassObject(JSContext *cx, JSObject *start, jsid id, jsval *vp) {
    ...
    if (!js_LookupPropertyWithFlags(cx, obj, id, JSRESOLVE_CLASSNAME, &pobj,
                                    &prop)) {
        return JS_FALSE;
    }
    if (!prop) {
...
JSObject *js_ConstructObject(JSContext *cx, JSClass *clasp, JSObject *proto,
...
static jsid CheckForStringIndex(jsid id, const jschar *cp, const jschar *end,
                                JSBool negative) {
...
static JSBool HidePropertyName(JSContext *cx, jsid *idp) {
    jsid id;
    JSAtom *atom, *hidden;
...
JSScopeProperty *js_AddHiddenProperty(JSContext *cx, JSObject *obj, jsid id,
                                      JSPropertyOp getter, JSPropertyOp setter,
                                      uint32 slot, uintN attrs, uintN flags,
                                      intN shortid) {
    if (!HidePropertyName(cx, &id))
        return NULL;

    flags |= SPROP_IS_HIDDEN;
    return js_AddNativeProperty(cx, obj, id, getter, setter, slot, attrs, flags,
                                shortid);
}

JSBool js_LookupHiddenProperty(JSContext *cx, JSObject *obj, jsid id,
                               JSObject **objp, JSProperty **propp) {
    return HidePropertyName(cx, &id) &&
           js_LookupPropertyWithFlags(cx, obj, id, JSRESOLVE_HIDDEN, objp,
                                      propp);
}

JSScopeProperty *js_AddNativeProperty(JSContext *cx, JSObject *obj, jsid id,
                                      JSPropertyOp getter, JSPropertyOp setter,
                                      uint32 slot, uintN attrs, uintN flags,
                                      intN shortid) {
    JSScope *scope;
...
JSScopeProperty *js_ChangeNativePropertyAttrs(JSContext *cx, JSObject *obj,
                                              JSScopeProperty *sprop,
...
JSBool js_DefineProperty(JSContext *cx, JSObject *obj, jsid id, jsval value,
                         JSPropertyOp getter, JSPropertyOp setter, uintN attrs,
                         JSProperty **propp) {
    return js_DefineNativeProperty(cx, obj, id, value, getter, setter, attrs, 0,
                                   0, propp);
}
...
JSBool js_DefineNativeProperty(JSContext *cx, JSObject *obj, jsid id,
                               jsval value, JSPropertyOp getter,
                               JSPropertyOp setter, uintN attrs, uintN flags,
                               intN shortid, JSProperty **propp) {
    JSClass *clasp;
    JSScope *scope;
    JSScopeProperty *sprop;
    ...
    if (attrs & (JSPROP_GETTER | JSPROP_SETTER)) {
        ...
        if (sprop && pobj == obj &&
            ...
            sprop = js_ChangeScopePropertyAttrs(
                cx, OBJ_SCOPE(obj), sprop, attrs, sprop->attrs,
                (attrs & JSPROP_GETTER) ? getter : sprop->getter,
                (attrs & JSPROP_SETTER) ? setter : sprop->setter);
 ...
 */
static JSBool Detecting(JSContext *cx, jsbytecode *pc) {
    JSScript *script;
    jsbytecode *endpc;
    JSOp op;
    JSAtom *atom;
    ...
    for (endpc = script->code + script->length; pc < endpc; pc++) {
        ...
        if (op == JSOP_NAME) {
            ...
            if (atom == cx->runtime->atomState.typeAtoms[JSTYPE_VOID] &&
                ...
                op = (JSOp)*pc;
                return op == JSOP_EQ || op == JSOP_NE || op == JSOP_NEW_EQ ||
                       op == JSOP_NEW_NE;
            }
            break;
        }
...
JSBool js_LookupPropertyWithFlags(JSContext *cx, JSObject *obj, jsid id,
            ...
            if (resolve != JS_ResolveStub) {
                ...
                *propp = NULL;
                ...
                if (clasp->flags & JSCLASS_NEW_RESOLVE) {
                    ...
                    if (!(flags & JSRESOLVE_CLASSNAME) && cx->fp &&
                        (pc = cx->fp->pc)) {
                        cs = &js_CodeSpec[*pc];
                        format = cs->format;
...
JSObject *js_FindIdentifierBase(JSContext *cx, jsid id) {
    ...
    if (JS_HAS_STRICT_OPTION(cx)) {
        ...
        if (!JS_ReportErrorFlagsAndNumber(
                cx, JSREPORT_WARNING | JSREPORT_STRICT, js_GetErrorMessage,
                NULL, JSMSG_UNDECLARED_VAR, JS_GetStringBytes(str))) {
            return NULL;
        }
    }
    return obj;
...
JSBool js_NativeGet(JSContext *cx, JSObject *obj, JSObject *pobj,
    ...
    slot = sprop->slot;
    *vp = (slot != SPROP_INVALID_SLOT) ? LOCKED_OBJ_GET_SLOT(pobj, slot)
                                       : JSVAL_VOID;
...
JSBool js_NativeSet(JSContext *cx, JSObject *obj, JSScopeProperty *sprop,
                    jsval *vp) {
    JSScope *scope;
    uint32 slot;
    jsval pval;
        ...
        *vp = JSVAL_VOID;
        ...
        if (JSVAL_IS_VOID(*vp) && cx->fp && (pc = cx->fp->pc)) {
            ...
            str = js_DecompileValueGenerator(cx, JSDVG_IGNORE_STACK,
                                             ID_TO_VALUE(id), NULL);
            if (!str || !JS_ReportErrorFlagsAndNumber(
                            cx, flags, js_GetErrorMessage, NULL,
                            JSMSG_UNDEFINED_PROP, JS_GetStringBytes(str))) {
                return JS_FALSE;
            }
        }
        return JS_TRUE;
...
JSBool js_SetProperty(JSContext *cx, JSObject *obj, jsid id, jsval *vp) {
    ...
    if (!sprop) {
        ...
        ADD_PROPERTY_HELPER(cx, clasp, obj, scope, sprop, vp,
                            js_RemoveScopeProperty(cx, scope, id);
                            JS_UNLOCK_SCOPE(cx, scope); return JS_FALSE);
...
read_only_error : {
    JSString *str = js_DecompileValueGenerator(cx, JSDVG_IGNORE_STACK,
                                               ID_TO_VALUE(id), NULL);
    if (!str)
        return JS_FALSE;
    return JS_ReportErrorFlagsAndNumberUC(cx, flags, js_GetErrorMessage, NULL,
                                          JSMSG_READ_ONLY,
                                          JS_GetStringChars(str));
}
}

JSBool js_GetAttributes(JSContext *cx, JSObject *obj, jsid id, JSProperty *prop,
                        uintN *attrsp) {
    JSBool noprop, ok;
    JSScopeProperty *sprop;
...
JSBool js_SetAttributes(JSContext *cx, JSObject *obj, jsid id, JSProperty *prop,
                        uintN *attrsp) {
    JSBool noprop, ok;
    JSScopeProperty *sprop;
    ...
    *rval = JSVAL_TRUE;
    ...
    if (sprop->attrs & JSPROP_PERMANENT) {
        ...
        str = js_DecompileValueGenerator(cx, JSDVG_IGNORE_STACK,
                                         ID_TO_VALUE(id), NULL);
        if (str) {
            JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL, JSMSG_PERMANENT,
...
JSBool js_DefaultValue(JSContext *cx, JSObject *obj, JSType hint, jsval *vp) {
    ...
    if (!JSVAL_IS_PRIMITIVE(v)) {
        ...
        *vp = OBJECT_TO_JSVAL(obj);
        ...
        if (str) {
            JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
                                 JSMSG_CANT_CONVERT_TO, JS_GetStringBytes(str),
                                 (hint == JSTYPE_VOID) ? "primitive type"
                                                       : js_type_strs[hint]);
        }
        return JS_FALSE;
    }
...
JSIdArray *js_NewIdArray(JSContext *cx, jsint length) {
    ...
    ida = (JSIdArray *)JS_malloc(cx, sizeof(JSIdArray) +
                                         (length - 1) * sizeof(jsval));
    if (ida)
        ida->length = length;
    return ida;
}
...
JSIdArray *js_SetIdArrayLength(JSContext *cx, JSIdArray *ida, jsint length) {
    ...
    rida = (JSIdArray *)JS_realloc(
        cx, ida, sizeof(JSIdArray) + (length - 1) * sizeof(jsval));
    if (!rida)
        JS_DestroyIdArray(cx, ida);
    else
        rida->length = length;
 ...
 * function.
 */
JSBool js_Enumerate(JSContext *cx, JSObject *obj, JSIterateOp enum_op,
                    jsval *statep, jsid *idp) {
    JSRuntime *rt;
    JSObject *proto;
    JSClass *clasp;
    ...
    case JSENUMERATE_INIT:
        ...
        state = (JSNativeIteratorState *)JS_malloc(
            cx, sizeof(JSNativeIteratorState));
        if (!state) {
            JS_DestroyIdArray(cx, ida);
            return JS_FALSE;
        }
...
#ifdef JS_THREADSAFE
void js_DropProperty(JSContext *cx, JSObject *obj, JSProperty *prop) {
    JS_UNLOCK_OBJ(cx, obj);
}
#endif

static void ReportIsNotFunction(JSContext *cx, jsval *vp, uintN flags) {
    /*
...
#ifdef NARCISSUS
static JSBool GetCurrentExecutionContext(JSContext *cx, JSObject *obj,
                                         jsval *rval) {
    JSObject *tmp;
    jsval xcval;
    ...
    while ((tmp = OBJ_GET_PARENT(cx, obj)) != NULL)
        obj = tmp;
    if (!OBJ_GET_PROPERTY(
            cx, obj, ATOM_TO_JSID(cx->runtime->atomState.ExecutionContextAtom),
            &xcval)) {
        return JS_FALSE;
    }
...
JSBool js_Call(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
               jsval *rval) {
    JSClass *clasp;
...
JSBool js_Construct(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    jsval *rval) {
    JSClass *clasp;
    ...
    if (!clasp->construct) {
        ...
        callee = JSVAL_TO_OBJECT(argv[-2]);
        if (!OBJ_GET_PROPERTY(
                cx, callee, ATOM_TO_JSID(cx->runtime->atomState.constructAtom),
                &cval)) {
            return JS_FALSE;
        }
...
JSBool js_HasInstance(JSContext *cx, JSObject *obj, jsval v, JSBool *bp) {
...
#ifdef NARCISSUS
    {
        ...
        if (!OBJ_GET_PROPERTY(
                cx, obj, ATOM_TO_JSID(cx->runtime->atomState.hasInstanceAtom),
                &fval)) {
            return JS_FALSE;
        }
        if (VALUE_IS_FUNCTION(cx, fval)) {
...
JSBool js_GetClassPrototype(JSContext *cx, JSObject *scope, jsid id,
                            JSObject **protop) {
    jsval v;
    JSObject *ctor;
    ...
    if (VALUE_IS_FUNCTION(cx, v)) {
        ...
        if (!OBJ_GET_PROPERTY(
                cx, ctor,
                ATOM_TO_JSID(cx->runtime->atomState.classPrototypeAtom), &v)) {
            return JS_FALSE;
        }
        if (!JSVAL_IS_PRIMITIVE(v)) {
            /*
 ...
 */
static JSBool CheckCtorGetAccess(JSContext *cx, JSObject *obj, jsval id,
                                 jsval *vp) {
    JSAtom *atom;
    uintN attrs;
    ...
    return OBJ_CHECK_ACCESS(cx, obj, ATOM_TO_JSID(atom), JSACC_READ, vp,
                            &attrs);
}

static JSBool CheckCtorSetAccess(JSContext *cx, JSObject *obj, jsval id,
                                 jsval *vp) {
    JSAtom *atom;
    uintN attrs;
    ...
    atom = cx->runtime->atomState.constructorAtom;
    JS_ASSERT(id == ATOM_KEY(atom));
    return OBJ_CHECK_ACCESS(cx, obj, ATOM_TO_JSID(atom), JSACC_WRITE, vp,
                            &attrs);
}

JSBool js_SetClassPrototype(JSContext *cx, JSObject *ctor, JSObject *proto,
                            uintN attrs) {
    /*
     ...
     * DontDelete.
     */
    if (!OBJ_DEFINE_PROPERTY(
            cx, ctor, ATOM_TO_JSID(cx->runtime->atomState.classPrototypeAtom),
            OBJECT_TO_JSVAL(proto), JS_PropertyStub, JS_PropertyStub, attrs,
            NULL)) {
        return JS_FALSE;
    }
     ...
     */
    return OBJ_DEFINE_PROPERTY(
        cx, proto, ATOM_TO_JSID(cx->runtime->atomState.constructorAtom),
        OBJECT_TO_JSVAL(ctor), CheckCtorGetAccess, CheckCtorSetAccess, 0, NULL);
}
...
JSBool js_TryMethod(JSContext *cx, JSObject *obj, JSAtom *atom, uintN argc,
                    jsval *argv, jsval *rval) {
...
JSBool js_XDRObject(JSXDRState *xdr, JSObject **objp) {
    ...
    if (xdr->mode == JSXDR_DECODE) {
        if (classDef) {
            ...
            protoKey = classDef >> 1;
            classKey = (protoKey != JSProto_Null) ? INT_TO_JSID(protoKey)
                                                  : ATOM_TO_JSID(atom);
    ...
    if (!clasp->xdrObject) {
        JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL, JSMSG_CANT_XDR_CLASS,
                             clasp->name);
        return JS_FALSE;
    }
...
static void MeterEntryCount(uintN count) {
    if (count) {
...
void js_DumpScopeMeters(JSRuntime *rt) {
    static FILE *logfp;
    if (!logfp)
...

    {
        ...
        fprintf(logfp, "scopes %g entries %g mean %g sigma %g max %u", nscopes,
                nentrys, mean, sigma, js_entry_count_max);
    }

    fprintf(
        logfp, " histogram %u %u %u %u %u %u %u %u %u %u %u\n",
        js_entry_count_hist[0], js_entry_count_hist[1], js_entry_count_hist[2],
        js_entry_count_hist[3], js_entry_count_hist[4], js_entry_count_hist[5],
        js_entry_count_hist[6], js_entry_count_hist[7], js_entry_count_hist[8],
        js_entry_count_hist[9], js_entry_count_hist[10]);
...
void printAtom(JSAtom *atom) { printString(ATOM_TO_STRING(atom)); }

...
#define JS_LOCK_OBJ(cx, obj)                                                   \
    ((OBJ_SCOPE(obj)->ownercx == (cx)) ? (void)0 : (js_LockObj(cx, obj)))
#define JS_UNLOCK_OBJ(cx, obj)                                                 \
    ((OBJ_SCOPE(obj)->ownercx == (cx)) ? (void)0 : js_UnlockObj(cx, obj))

#define JS_LOCK_SCOPE(cx, scope)                                               \
    ((scope)->ownercx == (cx) ? (void)0 : js_LockScope(cx, scope))
#define JS_UNLOCK_SCOPE(cx, scope)                                             \
    ((scope)->ownercx == (cx) ? (void)0 : js_UnlockScope(cx, scope))
...
#define JS_AWAIT_REQUEST_DONE(rt)                                              \
    JS_WAIT_CONDVAR((rt)->requestDone, JS_NO_TIMEOUT)

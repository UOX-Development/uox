...
#define STMT_TYPE_MAYBE_SCOPE(type)                                            \
    (type != STMT_WITH && STMT_TYPE_IN_RANGE(type, STMT_BLOCK, STMT_SUBROUTINE))
...
#define STMT_LINKS_SCOPE(stmt)                                                 \
    (STMT_TYPE_LINKS_SCOPE((stmt)->type) || ((stmt)->flags & SIF_SCOPE))
...
#define TREE_CONTEXT_INIT(tc)                                                  \
    ((tc)->flags = (tc)->numGlobalVars = 0,                                    \
     ...
     (tc)->topStmt = (tc)->topScopeStmt = NULL, (tc)->blockChain = NULL,       \
...
#define CG_PROLOG_OFFSET(cg)                                                   \
    PTRDIFF(CG_PROLOG_NEXT(cg), CG_PROLOG_BASE(cg), jsbytecode)
 ...
 * Emit one bytecode.
 */
extern ptrdiff_t js_Emit1(JSContext *cx, JSCodeGenerator *cg, JSOp op);

/*
 * Emit two bytecodes, an opcode (op) with a byte of immediate operand (op1).
 */
extern ptrdiff_t js_Emit2(JSContext *cx, JSCodeGenerator *cg, JSOp op,
                          jsbytecode op1);

/*
 * Emit three bytecodes, an opcode with two bytes of immediate operands.
 */
extern ptrdiff_t js_Emit3(JSContext *cx, JSCodeGenerator *cg, JSOp op,
                          jsbytecode op1, jsbytecode op2);

/*
 * Emit (1 + extra) bytecodes, for N bytes of op and its immediate operand.
 */
extern ptrdiff_t js_EmitN(JSContext *cx, JSCodeGenerator *cg, JSOp op,
                          size_t extra);
...
extern JSBool js_SetJumpOffset(JSContext *cx, JSCodeGenerator *cg,
                               jsbytecode *pc, ptrdiff_t off);

/* Test whether we're in a statement of given type. */
extern JSBool js_InStatement(JSTreeContext *tc, JSStmtType type);
 ...
 * otherwise.
 */
extern JSBool js_IsGlobalReference(JSTreeContext *tc, JSAtom *atom,
                                   JSBool *loopyp);

/*
 * Push the C-stack-allocated struct at stmt onto the stmtInfo stack.
 */
extern void js_PushStatement(JSTreeContext *tc, JSStmtInfo *stmt,
                             JSStmtType type, ptrdiff_t top);
 ...
 */
extern void js_PushBlockScope(JSTreeContext *tc, JSStmtInfo *stmt,
                              JSAtom *blockAtom, ptrdiff_t top);

/*
 * Pop tc->topStmt.  If the top JSStmtInfo struct is not stack-allocated, it
 * is up to the caller to free it.
 */
extern void js_PopStatement(JSTreeContext *tc);
 ...
 */
extern JSBool js_PopStatementCG(JSContext *cx, JSCodeGenerator *cg);
 ...
 */
extern JSBool js_DefineCompileTimeConstant(JSContext *cx, JSCodeGenerator *cg,
                                           JSAtom *atom, JSParseNode *pn);

extern JSBool js_LookupCompileTimeConstant(JSContext *cx, JSCodeGenerator *cg,
 ...
 */
extern JSStmtInfo *js_LexicalLookup(JSTreeContext *tc, JSAtom *atom,
                                    jsint *slotp, JSBool letdecl);

/*
 * Emit code into cg for the tree rooted at pn.
 */
extern JSBool js_EmitTree(JSContext *cx, JSCodeGenerator *cg, JSParseNode *pn);

/*
 * Emit function code into cg for the tree rooted at body.
 */
extern JSBool js_EmitFunctionBytecode(JSContext *cx, JSCodeGenerator *cg,
                                      JSParseNode *body);
 ...
 */
extern JSBool js_EmitFunctionBody(JSContext *cx, JSCodeGenerator *cg,
...
#define SN_TYPE(sn) (SN_IS_XDELTA(sn) ? SRC_XDELTA : *(sn) >> SN_DELTA_BITS)
...
#define SN_LENGTH(sn)                                                          \
    ((js_SrcNoteSpec[SN_TYPE(sn)].arity == 0) ? 1 : js_SrcNoteLength(sn))
 ...
 * memory.
 */
extern intN js_NewSrcNote(JSContext *cx, JSCodeGenerator *cg,
                          JSSrcNoteType type);

extern intN js_NewSrcNote2(JSContext *cx, JSCodeGenerator *cg,
                           JSSrcNoteType type, ptrdiff_t offset);

extern intN js_NewSrcNote3(JSContext *cx, JSCodeGenerator *cg,
                           JSSrcNoteType type, ptrdiff_t offset1,
                           ptrdiff_t offset2);

/*
 * NB: this function can add at most one extra extended delta note.
 */
extern jssrcnote *js_AddToSrcNoteDelta(JSContext *cx, JSCodeGenerator *cg,
                                       jssrcnote *sn, ptrdiff_t delta);

/*
 * Get and set the offset operand identified by which (0 for the first, etc.).
 */
extern JS_FRIEND_API(ptrdiff_t) js_GetSrcNoteOffset(jssrcnote *sn, uintN which);

extern JSBool js_SetSrcNoteOffset(JSContext *cx, JSCodeGenerator *cg,
                                  uintN index, uintN which, ptrdiff_t offset);
...
extern JSBool js_FinishTakingSrcNotes(JSContext *cx, JSCodeGenerator *cg,
                                      jssrcnote *notes);
 ...
 */
extern JSBool js_AllocTryNotes(JSContext *cx, JSCodeGenerator *cg);

/*
 * Grab the next trynote slot in cg, filling it in appropriately.
 */
extern JSTryNote *js_NewTryNote(JSContext *cx, JSCodeGenerator *cg,
...
extern void js_FinishTakingTryNotes(JSContext *cx, JSCodeGenerator *cg,
                                    JSTryNote *notes);

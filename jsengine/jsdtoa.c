...
struct Bigint {
    ...
    int32 sign; /* Zero if positive, 1 if negative.  Ignored by most Bigint
                   routines! */
    int32 wds;  /* Actual number of words.  If value is nonzero, the most
 ...
 */
JS_PUBLIC_API(int)
js_BigintTestingWhere() { return allocationNum; }
...
JS_strtod(CONST char *s00, char **se, int *err) {
    int32 scale;
    int32 bb2, bb5, bbe, bd2, bd5, bbbits, bs2, c, dsign, e, e1, esign, i, j, k,
        ...
        if (e1 &= ~15) {
            ...
            for (j = 0; e1 > 0; j++, e1 >>= 1)
                if (e1 & 1)
                    rv *= tinytens[j];
            if (scale &&
                (j = P + 1 - ((word0(rv) & Exp_mask) >> Exp_shift)) > 0) {
...
/* Return floor(b/S) and set b to be the remainder.  As added restrictions, b
 * must not have more words than S, the most significant word of S must not
 ...
 * expected. */
static JSBool js_dtoa(double d, int mode, JSBool biasUp, int ndigits,
                      int *decpt, int *sign, char **rve, char *buf,
                      size_t bufsize) {
    ...
    */
    ...
    int32 bbits, b2, b5, be, dig, i, ieps, ilim, ilim0, ilim1, j, j1, k, k0,
...
#else
        if ((i = (int32)(word0(d) >> Exp_shift1 & (Exp_mask >> Exp_shift1))) !=
            0) {
#endif
    d2 = d;
...
/* Now we have d/10^k = (b * 2^b2) / (2^s2 * 5^s5) and
   (mhi * 2^m2) / (2^s2 * 5^s5) = one-half of last printed or input significant
...
if (mode < 2) {
    if (!word1(d) && !(word0(d) & Bndry_mask)
        ...
        /* The special case.  Here we want to be within a quarter of the last
           input significant digit instead of one half of it when the decimal
           output string's value is less than d.  */
        b2 += Log2P;
        s2 += Log2P;
        spec_case = 1;
    }
...
if (ilim <= 0 && mode > 2) {
    /* We're doing fixed-mode output and d is less than the minimum nonzero
       output in this mode. Output either zero or the minimum nonzero output
       depending on which is closer to d. */
    if (ilim < 0)
        goto no_digits;
    S = multadd(S, 5, 0);
    if (!S)
...
if (leftright) {
    ...
    /* mlo/S = maximum acceptable error, divided by 10^k, if the output is less
     * than d. */
        ...
        if ((j < 0) || (j == 0 && !mode
            ...
            if (j1 > 0) {
                /* Either dig or dig+1 would work here as the least significant
                   decimal digit. Use whichever would produce a decimal value
                   closer to d. */
                b = lshift(b, 1);
                if (!b)
                    goto nomem;
...
static const int dtoaModes[] = {0,  /* DTOSTR_STANDARD */
...
JS_FRIEND_API(char *)
JS_dtostr(char *buffer, size_t bufferSize, JSDToStrMode mode, int precision,
          double d) {
    ...
    char *numBegin = buffer + 2; /* Pointer to the digits returned by js_dtoa;
                                    the +2 leaves space for */
                                 /* the sign and/or decimal point */
    char *numEnd; /* Pointer past the digits returned by js_dtoa */
    JSBool dtoaRet;

    JS_ASSERT(bufferSize >=
              (size_t)(mode <= DTOSTR_STANDARD_EXPONENTIAL
                           ? DTOSTR_STANDARD_BUFFER_SIZE
                           : DTOSTR_VARIABLE_BUFFER_SIZE(precision)));

    if (mode == DTOSTR_FIXED && (d >= 1e21 || d <= -1e21))
        mode = DTOSTR_STANDARD; /* Change mode here rather than below because
                                   the buffer may not be large enough to hold a
                                   large integer. */

    /* Locking for Balloc's shared buffers */
    ACQUIRE_DTOA_LOCK();
    dtoaRet = js_dtoa(d, dtoaModes[mode], mode >= DTOSTR_FIXED, precision,
                      &decPt, &sign, &numEnd, numBegin, bufferSize - 2);
    RELEASE_DTOA_LOCK();
    if (!dtoaRet)
        return 0;
    ...
    if (decPt != 9999) {
        ...
        int minNDigits = 0; /* Minimum number of significand digits required by
                               mode and precision */
        char *p;
        char *q;
...
static uint32 divrem(Bigint *b, uint32 divisor) {
    int32 n = b->wds;
    uint32 remainder = 0;
    ULong *bx;
    ULong *bp;
    ...
    if (buffer) {
        ...
        if (d < 0.0
            ...
            !((word0(d) & Exp_mask) == Exp_mask &&
              ((word0(d) & Frac_mask) ||
               word1(
                   d))) /* Visual C++ doesn't know how to compare against NaN */
#endif
        ) {
            *p++ = '-';
            d = -d;
        ...
        if ((word0(d) & Exp_mask) == Exp_mask) {
            strcpy(p,
                   !word1(d) && !(word0(d) & Frac_mask) ? "Infinity" : "NaN");
            return buffer;
        }
                ...
                if (j1 == 0 && !(word1(d) & 1)) {
                    ...
                    if (j1 > 0) {
                        ...
                        if (!b)
                            goto nomem2;
                        j1 = cmp(b, s);
                        if (j1 > 0) /* The even test (|| (j1 == 0 && (digit &

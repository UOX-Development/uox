 ...
 */
static JSBool PutBlockObjects(JSContext *cx, JSStackFrame *fp) {
    JSBool ok;
    JSObject *obj;
...
static JSBool NoSuchMethod(JSContext *cx, JSStackFrame *fp, jsval *vp,
                           uint32 flags, uintN argc) {
...
static void *AllocCallTable(void *pool, size_t size) { return malloc(size); }

static void FreeCallTable(void *pool, void *item) { free(item); }

static JSHashEntry *AllocCallEntry(void *pool, const void *key) {
    return (JSHashEntry *)calloc(1, sizeof(CallEntry));
}

static void FreeCallEntry(void *pool, JSHashEntry *he, uintN flag) {
    JS_ASSERT(flag == HT_FREE_ENTRY);
    free(he);
}

static JSHashAllocOps callTableAllocOps = {AllocCallTable, FreeCallTable,
...
CallTableDumper(JSHashEntry *he, intN k, void *arg) {
    ...
    fprintf(fp, "calls %lu (%lu) argc %u/%u\n", (unsigned long)ce->value.total,
            (unsigned long)ce->value.recycled, ce->value.minargc,
            ce->value.maxargc);
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (n == 1) {
            fprintf(fp, "  arg %u type %s: %lu\n", i, TYPENAME(save),
                    (unsigned long)ai->typeHist[save]);
        } else {
            fprintf(fp, "  arg %u type histogram:\n", i);
            for (type = JSTYPE_VOID; type <= JSTYPE_LIMIT; type++) {
                fprintf(fp, "  %9s: %8lu ", TYPENAME(type),
                        (unsigned long)ai->typeHist[type]);
        ...
        for (cl = ai->lruList.prev; cl != &ai->lruList; cl = cl->prev) {
            ...
            fprintf(fp, "  %9u: %8lu %.*s (%#lx)\n", n,
                    (unsigned long)avc->count, sizeof avc->strbuf, avc->strbuf,
                    argval);
            ++n;
        }
    }
...
void js_DumpCallTable(JSContext *cx) {
    char name[24];
    FILE *fp;
...
static void LogCall(JSContext *cx, jsval callee, uintN argc, jsval *argv) {
    CallKey key;
    ...
    if (!js_CallTable) {
        js_CallTable =
            JS_NewHashTable(1024, js_hash_call_key, js_compare_call_keys, NULL,
...
JSBool js_InternalInvoke(JSContext *cx, JSObject *obj, jsval fval, uintN flags,
...
JSBool js_InternalGetOrSet(JSContext *cx, JSObject *obj, jsid id, jsval fval,
                           JSAccessMode mode, uintN argc, jsval *argv,
                           jsval *rval) {
    int stackDummy;
    ...
    if (cx->runtime->checkObjectAccess && VALUE_IS_FUNCTION(cx, fval) &&
        FUN_INTERPRETED(
            (JSFunction *)JS_GetPrivate(cx, JSVAL_TO_OBJECT(fval))) &&
 ...
 */
static JSBool ImportProperty(JSContext *cx, JSObject *obj, jsid id) {
    JSBool ok;
    JSIdArray *ida;
    JSProperty *prop;
    ...
    do {
        ...
        if (prop && target == obj2) {
            ...
            ok = OBJ_DEFINE_PROPERTY(
                cx, target, id, value, NULL, NULL,
                attrs & ~(JSPROP_EXPORTED | JSPROP_GETTER | JSPROP_SETTER),
                NULL);
        }
        if (prop)
            OBJ_DROP_PROPERTY(cx, obj2, prop);
...
JSBool js_CheckRedeclaration(JSContext *cx, JSObject *obj, jsid id, uintN attrs,
    ...
    type = (oldAttrs & attrs & JSPROP_GETTER)   ? js_getter_str
           : (oldAttrs & attrs & JSPROP_SETTER) ? js_setter_str
           : (oldAttrs & JSPROP_READONLY)       ? js_const_str
           : isFunction                         ? js_function_str
                                                : js_var_str;
    name = js_AtomToPrintableString(cx, JSID_TO_ATOM(id));
    if (!name)
        goto bad;
    return JS_ReportErrorFlagsAndNumber(cx, report, js_GetErrorMessage, NULL,
...
static JSBool InternStringElementId(JSContext *cx, jsval idval, jsid *idp) {
    JSAtom *atom;
...
static JSBool InternNonIntElementId(JSContext *cx, jsval idval, jsid *idp) {
 ...
 */
#if JS_VERSION >= 160 &&                                                       \
    (__GNUC__ >= 3 || (__IBMC__ >= 700 && defined __IBM_COMPUTED_GOTO) ||      \
     __SUNPRO_C >= 0x570)
...
#define OPDEF(op, val, name, token, length, nuses, ndefs, prec, format)        \
    ((op != JSOP_PUSHOBJ) ? JS_EXTENSION && interrupt                          \
...
#define LOAD_INTERRUPT_HANDLER(rt)                                             \
    ...
    interrupt:
        SAVE_SP_AND_PC(fp);
        switch (
            interruptHandler(cx, script, pc, &rval, rt->interruptHandlerData)) {
        case JSTRAP_ERROR:
            ok = JS_FALSE;
            goto out;
...
#define FETCH_ELEMENT_ID(n, id)                                                \
    ...
    if (JSVAL_IS_PRIMITIVE(rval)) {
        ...
        if (str) {
            JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
                                 JSMSG_IN_NOT_OBJECT, JS_GetStringBytes(str));
        }
        ok = JS_FALSE;
        goto out;
    }
...
#define END_LITOPX_CASE(OP) END_CASE(OP)
    ...
    SAVE_SP_AND_PC(fp);
    ok = OBJ_DEFINE_PROPERTY(
        cx, obj, ATOM_TO_JSID(atom), rval, NULL, NULL,
        JSPROP_ENUMERATE | JSPROP_PERMANENT | JSPROP_READONLY, NULL);
    if (!ok)
        goto out;
    STORE_OPND(-1, rval);
...
#define XML_EQUALITY_OP(OP)                                                    \
    if ((ltmp == JSVAL_OBJECT && (obj2 = JSVAL_TO_OBJECT(lval)) &&             \
         OBJECT_IS_XML(cx, obj2)) ||                                           \
        (rtmp == JSVAL_OBJECT && (obj2 = JSVAL_TO_OBJECT(rval)) &&             \
         OBJECT_IS_XML(cx, obj2))) {                                           \
        JSXMLObjectOps *ops;                                                   \
                                                                               \
...
do_incop : {
    ...
    if (JSVAL_IS_INT(rval) && rval != INT_TO_JSVAL(JSVAL_INT_MIN) &&
...
do_nonint_fast_global_incop : {
    ...
    atom = GET_ATOM(cx, script, pc);
    lval = FETCH_OPND(-1);
    if (JSVAL_IS_STRING(lval) && atom == cx->runtime->atomState.lengthAtom) {
...
#define LAZY_ARGS_THISP ((JSObject *)JSVAL_VOID)
    ...
    if (VALUE_IS_FUNCTION(cx, lval) &&
        (obj = JSVAL_TO_OBJECT(lval),
         fun = (JSFunction *)JS_GetPrivate(cx, obj), FUN_INTERPRETED(fun)))
    /* inline_call: */
    {
        ...
        if (missing) {
            ...
            overflow = JS_FALSE; /* suppress bogus gcc warnings */
#endif
        ...
        if (avail <= a->limit) {
            ...
            if (!newsp) {
                JS_ReportErrorNumber(
                    ...
                    (fp && fp->fun) ? JS_GetFunctionName(fp->fun) : "script");
        ...
        newifp->frame.thisp = js_ComputeThis(cx,
        ...
        if (hook) {
            ...
            newifp->hookData =
                hook(cx, &newifp->frame, JS_TRUE, 0, rt->callHookData);
            LOAD_INTERRUPT_HANDLER(rt);
        } else {
...
do_JSOP_OBJECT:
    ...
    if (!JSVAL_IS_NUMBER(lval) && !JSVAL_IS_STRING(lval) &&
...
#define SEARCH_PAIRS(MATCH_CODE)                                               \
    ...
    if (JSVAL_IS_STRING(lval)) {
        str = JSVAL_TO_STRING(lval);
        SEARCH_PAIRS(match = (JSVAL_IS_STRING(rval) &&
...
#define SEARCH_EXTENDED_PAIRS(MATCH_CODE)                                      \
    ...
    if (JSVAL_IS_STRING(lval)) {
        str = JSVAL_TO_STRING(lval);
        SEARCH_EXTENDED_PAIRS(match = (JSVAL_IS_STRING(rval) &&
                                       ((str2 = JSVAL_TO_STRING(rval)) == str ||
                                        js_EqualStrings(str2, str)));)
    } else if (JSVAL_IS_DOUBLE(lval)) {
        d = *JSVAL_TO_DOUBLE(lval);
        ...
        SEARCH_EXTENDED_PAIRS(match = (lval == rval);)
    }
...
do_JSOP_DEFVAR:
    ...
    if (!prop) {
        ok = OBJ_DEFINE_PROPERTY(cx, obj, id, JSVAL_VOID, NULL, NULL, attrs,
                                 &prop);
        if (!ok)
            goto out;
        JS_ASSERT(prop);
     ...
     */
    if (atomIndex < script->numGlobalVars && (attrs & JSPROP_PERMANENT) &&
        obj2 == obj && OBJ_IS_NATIVE(obj)) {
        sprop = (JSScopeProperty *)prop;
    ...
    if (ok) {
        ok = OBJ_DEFINE_PROPERTY(
            cx, parent, id, rval,
            (flags & JSPROP_GETTER) ? JS_EXTENSION(JSPropertyOp) obj : NULL,
            (flags & JSPROP_SETTER) ? JS_EXTENSION(JSPropertyOp) obj : NULL,
            attrs, &prop);
    }
    ...
    ok = OBJ_DEFINE_PROPERTY(
        cx, parent, ATOM_TO_JSID(fun->atom), rval,
        (attrs & JSPROP_GETTER) ? JS_EXTENSION(JSPropertyOp) obj : NULL,
        (attrs & JSPROP_SETTER) ? JS_EXTENSION(JSPropertyOp) obj : NULL,
        attrs | JSPROP_ENUMERATE | JSPROP_PERMANENT | JSPROP_READONLY, NULL);
    ...
    ok = OBJ_DEFINE_PROPERTY(
        ...
        (attrs & JSPROP_GETTER) ? JS_EXTENSION(JSPropertyOp) obj : NULL,
        (attrs & JSPROP_SETTER) ? JS_EXTENSION(JSPropertyOp) obj : NULL,
        attrs | JSPROP_ENUMERATE | JSPROP_PERMANENT, &prop);
    ...
    if (JS_TypeOfValue(cx, rval) != JSTYPE_FUNCTION) {
        JS_ReportErrorNumber(
            cx, js_GetErrorMessage, NULL, JSMSG_BAD_GETTER_OR_SETTER,
    ...
    ok = OBJ_DEFINE_PROPERTY(cx, obj, id, JSVAL_VOID, getter, setter, attrs,
                             NULL);
    if (!ok)
        goto out;
...
do_init:
    ...
    if (JSVAL_IS_PRIMITIVE(rval)) {
        ...
        JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL, JSMSG_BAD_SHARP_DEF,
                             numBuf);
        ok = JS_FALSE;
        goto out;
    }
    ...
    if (!JSVAL_IS_OBJECT(rval)) {
        ...
        SAVE_SP_AND_PC(fp);
        JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL, JSMSG_BAD_SHARP_USE,
                             numBuf);
        ok = JS_FALSE;
        goto out;
    }
    ...
    for (obj = fp->blockChain; obj; obj = OBJ_GET_PARENT(cx, obj)) {
        ...
        if (OBJ_BLOCK_DEPTH(cx, obj) + (jsint)OBJ_BLOCK_COUNT(cx, obj) <= i) {
            JS_ASSERT(OBJ_BLOCK_DEPTH(cx, obj) < i ||
                      OBJ_BLOCK_COUNT(cx, obj) == 0);
            break;
        }
    }
    fp->blockChain = obj;
    ...
    for (obj = fp->scopeChain;
        ...
        if (JS_GetPrivate(cx, obj) != fp || OBJ_BLOCK_DEPTH(cx, obj) < i) {
            break;
        }
    ...
    BEGIN_CASE(JSOP_DEBUGGER) {
        ...
        if (handler) {
            SAVE_SP_AND_PC(fp);
            switch (handler(cx, script, pc, &rval, rt->debuggerHandlerData)) {
            case JSTRAP_ERROR:
                ok = JS_FALSE;
                goto out;
            case JSTRAP_CONTINUE:
    ...
    *result = POP_OPND();
    ...
    str2 = JSVAL_TO_STRING(rval);
    SAVE_SP_AND_PC(fp);
    obj = js_NewXMLSpecialObject(cx, JSXML_CLASS_PROCESSING_INSTRUCTION, str,
                                 str2);
    if (!obj) {
        ok = JS_FALSE;
        goto out;
    ...
    if (!JSVAL_IS_PRIMITIVE(lval)) {
        ...
        if (JSVAL_IS_STRING(lval)) {
            ...
            str =
                js_DecompileValueGenerator(cx, JSDVG_SEARCH_STACK, lval, NULL);
            if (str) {
                JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
                                     JSMSG_NO_PROPERTIES,
                                     JS_GetStringBytes(str));
    ...
    if (fp->flags & JSFRAME_POP_BLOCKS) {
        ...
        JS_ASSERT(!fp->blockChain || OBJ_GET_PARENT(cx, obj) == fp->blockChain);
        fp->blockChain = obj;
    }
    ...
    BEGIN_CASE(JSOP_LEAVEBLOCK) {
        ...
        *chainp = OBJ_GET_PARENT(cx, obj);
        JS_ASSERT(chainp != &fp->blockChain || !*chainp ||
...
#undef FAST_LOCAL_INCREMENT_OP
    ...
    *--sp = JSVAL_NULL;
    ...
    if (FRAME_TO_GENERATOR(fp)->state == JSGEN_CLOSING) {
        str = js_DecompileValueGenerator(cx, JSDVG_SEARCH_STACK, fp->argv[-2],
                                         NULL);
        if (str) {
            JS_ReportErrorNumberUC(cx, js_GetErrorMessage, NULL,
...
#endif
{
    ...
    JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL, JSMSG_BAD_BYTECODE,
                         numBuf);
    ok = JS_FALSE;
    goto out;
}
...
if (tracefp) {
    ...
    if (ndefs) {
        ...
        for (n = -ndefs; n < 0; n++) {
            str = js_DecompileValueGenerator(cx, n, sp[n], NULL);
            if (str) {
                fprintf(tracefp, "%s %s", (n == -ndefs) ? "  output:" : ",",
                        JS_GetStringBytes(str));
            }
        }
        fprintf(tracefp, " @ %d\n", sp - fp->spbase);
    ...
    for (siter = fp->spbase; siter < sp; siter++) {
        ...
        fprintf(tracefp, "%s ", str ? JS_GetStringBytes(str) : "<null>");
    }
    fputc('\n', tracefp);
}
#endif /* DEBUG */
...
out2 : if (cx->version == currentVersion && currentVersion != originalVersion)

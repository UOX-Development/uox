 ...
 * resulting ASCII string. The result is held somewhere in buffer, but not
 * necessarily at the beginning.  The size of buffer is given in bufferSize, and
 * must be at least as large as given by the above macros.
 *
 ...
 * printed exactly in that base, regardless of how large it is, because there is
 * no exponential notation for non-base-ten numbers.  The fractional part will
 * be rounded to as few digits as possible while still preserving the round-trip
 * property (analogous to that of printing decimal numbers).  In other words, if
 * one were to read the resulting string in via a hypothetical
 * base-number-reading routine that rounds to the nearest IEEE double (and to an
 * even significand if there are two equally near doubles), then the result
 * would equal d (except for -0.0, which converts to "0", and NaN, which is not
 * equal to itself).
 *

...
#if defined(_WIN32)
const std::string uodefaultdir = "C:\\Program Files (x86)\\Electronc Arts\\Ultima Online Classic";
#else
...
// Constants used
//=================================================================================
const std::unordered_map<relativepath_t,std::filesystem::path> config_t::relativepath = {
...
auto config_t::clear() ->void {
    ...
    datalocation.clear();
    filenames.clear() ;
    datalocation[datalocation_t::server] = std::filesystem::current_path() / std::filesystem::path("server_data");
    datalocation[datalocation_t::shard] = std::filesystem::current_path() / std::filesystem::path("shard_data");
...
//=================================================================================
auto config_t::process(const std::string &key, const std::string &value,int linenumber) ->void{
    ...
    if (lkey == "port"){
        ...
        filenames[filepath_t::account] = std::filesystem::path(value).filename() ;
    }
    else if (lkey == "firewall_file") {
        filenames[filepath_t::firewall] = std::filesystem::path(value).filename() ;
    }
   else {
        std::clog <<"In configuration, line: "<<linenumber<<" uknown key: '"<<key<<"' and value: '"<<value<<"'"<<std::endl;
    }
}
//=================================================================================
config_t::config_t(const std::filesystem::path &configpath ):port(2593),public_ip("localhost") {
    clear();
    if (!configpath.empty()){
        load(configpath) ;
    }
...
auto config_t::location(relativepath_t path,datalocation_t location) const ->std::filesystem::path {
...
auto config_t::filepath(filepath_t path,datalocation_t location) const ->std::filesystem::path{

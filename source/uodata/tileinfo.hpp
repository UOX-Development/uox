...
struct art_info_t : public info_t {
    ...
    art_info_t():info_t(infotype_t::art),weight(0),quality(0),miscdata(0),unknown2(0),quantity(0),animid(0xFFFF),unknown3(0),hue(0),stackingoffset(0),height(0){}
    auto load(std::istream &input) ->void final ;
};
...
struct tileinfo_t {
private:
    std::map<std::uint16_t,std::vector<std::pair<std::string,std::string>>> terrain_modifications;
    std::map<std::uint16_t,std::vector<std::pair<std::string,std::string>>> art_modifications;
    auto updateAttribute(bool isart,std::uint16_t id, const std::string &key, const std::string &value)->void ;
    auto updateFlagAttribute(bool isart,std::uint16_t id, const std::string &key, bool shouldset)->void ;

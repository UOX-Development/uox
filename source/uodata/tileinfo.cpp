...
static const std::unordered_map<std::string, flag_t> flagnames{
    {"background"s, uo::background},
    {"holdable"s, uo::holdable},
    {"transparent"s, uo::transparent},
    {"translucent"s, uo::translucent},
    {"wall"s, uo::wall},
    {"damaging"s, uo::damaging},
    {"impassable"s, uo::impassable},
    {"wet"s, uo::wet},
    {"bit9"s, uo::bit9},
    {"surface"s, uo::surface},
    {"bridge"s, uo::bridge},
    {"generic"s, uo::generic},
    {"window"s, uo::window},
    {"noshoot"s, uo::noshoot},
    {"articlea"s, uo::articlea},
    {"articlean"s, uo::articlean},
    {"articlethe"s, uo::articlethe},
    {"foilage"s, uo::foilage},
    {"partialhue"s, uo::partialhue},
    {"nohouse"s, uo::nohouse},
    {"map"s, uo::map},
    {"container"s, uo::container},
    {"wearable"s, uo::wearable},
    {"lightsource"s, uo::lightsource},
    {"animation"s, uo::animation},
    {"hoverover"s, uo::hoverover},
    {"nodiagonal"s, uo::nodiagonal},
    {"armor"s, uo::armor},
    {"roof"s, uo::roof},
    {"door"s, uo::door},
    {"stairback"s, uo::stairback},
    {"stairright"s, uo::stairright},
    {"alphablend"s, uo::alphablend},
    {"usenewart"s, uo::usenewart},
    {"artused"s, uo::artused},
    {"bit36"s, uo::bit36},
    {"noshadow"s, uo::noshadow},
    {"pixelbleed"s, uo::pixelbleed},
    {"playanimonce"s, uo::playanimonce},
    {"bit40"s, uo::bit40},
    {"multimovable"s, uo::multimovable},
    {"bit42"s, uo::bit42},
    {"bit45"s, uo::bit45},
    {"bit46"s, uo::bit46},
    {"bit47"s, uo::bit47},
...
//==============================================================
auto uoflag_t::set(flag_t value) -> void { flag |= value; }
//==============================================================
auto uoflag_t::clear(flag_t value) -> void { flag &= (~value); }
...
auto art_info_t::load(std::istream &input) -> void {
    ...
    input.read(reinterpret_cast<char *>(&stackingoffset),
               sizeof(stackingoffset));
...
//==============================================================
//==============================================================
auto tileinfo_t::updateFlagAttribute(bool isart, std::uint16_t id,
                                     const std::string &key, bool shouldset)
    -> void {
    try {
        ...
        throw std::runtime_error(
            strutil::format("For %s tile %i, unable to set/clear flag(s): %s",
                            name.c_str(), id, key.c_str()));
    }
}
//==============================================================
auto tileinfo_t::updateAttribute(bool isart, std::uint16_t id,
    ...
    try {
        if (!isart) {
            if (key == "name") {
                ...
                terrain.at(id).texture =
                    static_cast<std::uint16_t>(std::stoul(value, nullptr, 0));
            }
        } else {
            if (key == "name") {
                ...
                art.at(id).weight =
                    static_cast<std::uint8_t>(std::stoul(value, nullptr, 0));
            } else if (key == "quality") {
                art.at(id).quality =
                    static_cast<std::uint8_t>(std::stoul(value, nullptr, 0));
        ...
        auto name = (isart ? "Art"s : "Terrain"s);
        throw std::runtime_error(
            strutil::format("%s tile modification failed, Key: %s Value: %s",
...
auto tileinfo_t::load(const std::filesystem::path &uopath) -> tileinfo_t & {
    ...
    for (auto i = 0; i < 0x4000; i++) {
        ...
        if ((((i & 0x1F) == 0) && (i > 0)) || (i == 1)) {
            input.read(reinterpret_cast<char *>(&dummy),
                       4); // read off the group header
        }
        if (!input.good() || input.eof()) {
            throw std::runtime_error("Error processing: "s + infopath.string());
        }
...
auto tileinfo_t::loadUpdates(const std::filesystem::path &infofile) -> bool {
        ...
        if (input.gcount() > 0) {
            ...
            if (!curline.empty()) {
                ...
                case search_t::header: {
                    if (curline.size() > 2) {
                        if ((curline[0] == '[') &&
                            (curline[curline.size() - 1] == ']')) {
                            auto [id, type] = strutil::split(
                                curline.substr(1, curline.size() - 2), ":");
                            try {
                                tileid = std::stoi(id, nullptr, 0);
                            } catch (...) {
                                // The id isn't something we recoginize as a
                                // valid number
                                throw std::runtime_error(strutil::format(
                                    "Line: %i Invalid header: %s File: %s",
                                    linenumber, curline.c_str(),
                                    infofile.string().c_str()));
                            }
                            ...
                            // We get to here, we can start searching for
                            // section start
                ...
                case search_t::start: {
                    if (curline != "{") {
                        throw std::runtime_error(strutil::format(
                            "Line: %i Expected start of section: %s File: %s",
                            linenumber, curline.c_str(),
                            infofile.string().c_str()));
                    }
                ...
                case search_t::data: {
                    if ((curline[0] == '[') || (curline[0] == '{')) {
                        throw std::runtime_error(
                            strutil::format("Line: %i Unexpected value(looking "
                                            "for data): %s File: %s",
                                            linenumber, curline.c_str(),
                                            infofile.string().c_str()));
                    }
                    if (curline != "}") {
                        ...
                        data.push_back(
                            std::make_pair(strutil::lower(key), value));
                    } else {
                        if (isart) {
                            art_modifications.insert_or_assign(tileid, data);
                        } else {
                            terrain_modifications.insert_or_assign(tileid,
                                                                   data);
                        }
                        data.clear();
    ...
    if (search != search_t::header) {
        throw std::runtime_error(
            strutil::format("Line: %i Incomplete section in file: %s",
                            linenumber, infofile.string().c_str()));
    }
    return true;
}
//==============================================================
